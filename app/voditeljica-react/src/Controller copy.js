...
function App() {
  ...
  return (
    <AppContainer>
      ...
      <Controls>
        <Button color={'#78E0DC'} onClick={() => fetch(`${window.location.origin.toString()}/playFirst/${whichSelected()}`)}>1. vprašanje</Button>
        <Button color={'lightblue'} onClick={() => fetch(`${window.location.origin.toString()}/startClock`)}>odštevanje</Button>
        <Button color={'#F4998D'} style={{display: 'inline'}} onClick={() => fetch(`${window.location.origin.toString()}/failed`)}>nepravilno</Button>
        <Button color={'#78E0DC'} style={{display: 'inline'}}onClick={() => fetch(`${window.location.origin.toString()}/playSecond/${whichSelected()}`)}>2. vprašanje</Button>
        <Button color={'lightblue'} onClick={() => fetch(`${window.location.origin.toString()}/startClock`)}>odštevanje</Button>
        ...
        }}>zapri kvadrat</Button>
        <Button color={'#78E0DC'} style={{display: 'inline'}}onClick={() => fetch(`${window.location.origin.toString()}/playMain`)}>zmagovalno vprašanje</Button>
        <Button color={'lightblue'} onClick={() => fetch(`${window.location.origin.toString()}/startClock`)}>odštevanje</Button>
        <Button color={'#F4998D'} onClick={() => {        
          setIsClosed(isClosed.map((item, iter) => {
            if (isSelected[iter]) return true
