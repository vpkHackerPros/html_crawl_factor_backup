...
const BallControl = ( props ) => {
  ...
  const OneBallControl = ( props ) => <>
      {/*}<MiniLabel>kam je padla</MiniLabel>
      <ButtonRow labels={props.prizes} onClicks={props.prizes.map((prize, iter) => () => {
        console.log(prize)
      })}/>
    <MiniLabel>kam bo padla</MiniLabel>{*/}
      <ButtonRow labels={props.prizes} onClicks={props.prizes.map((prize, iter) => () => {
        props.setMoney(props.order, prize)
        fetch(`http://localhost:4545/ballDropNeutral/${iter}/${isColored}`)
      })}/>
  </>
  return (
    <Container>
      ...
      <ButtonRow labels={[ 'spust ANIM GFX', 'modro ozadje', 'resetiraj barve kontejnerjev']} onClicks={[
